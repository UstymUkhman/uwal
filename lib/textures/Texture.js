...
export default class Texture
{
     ...
     * @typedef {Filters | Filters[keyof Filters]} Filter
     *
    ...
    /**
     * @param {
           Pick<Partial<GPUTextureDescriptor>, "format" | "usage"> &
    ...
    /**
     * @param {
           Pick<Partial<GPUTextureDescriptor>, "format" | "usage" | "size"> &
    ...
    async #CreateMipmaps(texture, descriptor, loop)
    {
        ...
        const { UseDepthStencilAttachment, RenderPassDescriptor } = this.#Renderer;

        const pipelinesState = this.#Renderer.Pipelines.map(Pipeline => {
        ...
        this.#Renderer.UseDepthStencilAttachment = false;
        ...
        this.#Renderer.UseDepthStencilAttachment = UseDepthStencilAttachment;
        pipelinesState.forEach((state, s) => this.#Renderer.Pipelines[s].Active = state);

        const depthStencilAttachment = RenderPassDescriptor.depthStencilAttachment &&
            this.#Renderer.CreateDepthStencilAttachment(
                ...
                RenderPassDescriptor.depthStencilAttachment.stencilAttachment
            );
