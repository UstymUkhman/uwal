...
export default class Texture
{
    ...
    CreateStorageTexture(descriptor = {})
    {
        ...
        let size = /** @type {GPUExtent3DStrict} */ (descriptor.size);
    ...
    CreateTextureFromSource(source, options = {})
    {
        ...
        const size = /** @type {GPUExtent3DStrict} */ (Array.isArray(options.size) || !options.size
    ...
    async #CreateMipmaps(texture, descriptor, loop)
    {
        ...
        await Renderer.AddPipeline(MipmapsPipeline,
        {
            ...
            fragment: MipmapsPipeline.CreateFragmentState(this.#MipmapsModule,
    ...
    async GenerateCubeMipmaps(texture)
    {
        return this.#CreateMipmaps(texture, { minMagFilter: FILTER.LINEAR }, (Pipeline, baseMipLevel) =>
        {
            for (let l = 0; l < texture.depthOrArrayLayers; ++l)
            {
                ...
                ], [BINDINGS.MIPMAP_SAMPLER, BINDINGS.MIPMAP_TEXTURE]);
