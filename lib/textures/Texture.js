...
export default class Texture
{
     ...
     * @typedef {Addresses[keyof Addresses]} Address
     * @typedef {Filters[keyof Filters]} Filter
     *
    ...
    /**
     * @typedef {GPUTexelCopyTextureInfo & GPUTexelCopyBufferLayout & OptionalGPUExtent3DStrict} WriteOptions
     ...
     * @param {GPUCopyExternalImageSource | GPUTexture} source
     * @param {TextureDescriptor | boolean} [options = { mipmaps: true }]
     */
    CreateTextureFromSource(source, options = {})
    {
        ...
        const size = /** @type {GPUExtent3D} */ (Array.isArray(options.size) || !options.size
    ...
    /**
     * @param {string} source
     * @param {ImageBitmapOptions} [options = { colorSpaceConversion: "none" }]
     ...
     * @param {CopyOptions} [options = { create: true, flipY: true }]
     */
    async CopyImageToTexture(source, options = { create: true, flipY: true })
    {
        ...
        if ((mipmaps ?? true) && 1 < texture.mipLevelCount)
            await (texture.depthOrArrayLayers === 1 ? this.GenerateMipmaps(texture) : this.GenerateCubeMipmaps(texture));
     ...
     * @param {ImageBitmapOptions} [bitmapOptions = { colorSpaceConversion: "none" }]
     * @param {TextureDescriptor} [textureOptions = { mipmaps: true }]
     * @param {CopyOptions} [copyOptions = { flipY: false }]
     * @param {RequestInit} [requestOptions]
     */
    async CreateCubeTexture(sources, bitmapOptions, textureOptions, copyOptions, requestOptions)
    {
     ...
     * @param {SamplerDescriptor} descriptor
     * @param {(Pipeline: RenderPipeline, baseMipLevel: number) => void} loop
     */
    async #CreateMipmaps(texture, descriptor, loop)
    {
        ...
        await Renderer.AddPipeline(MipmapsPipeline,
        {
            ...
            fragment: MipmapsPipeline.CreateFragmentState(this.#MipmapsModule, "fragment",
    ...
    /**
     * @param {GPUTexelCopyBufferInfo & Partial<GPUTexelCopyTextureInfo> & OptionalGPUExtent3DStrict & {
     *     create?: TextureDescriptor | boolean;
     *     source?: GPUCopyExternalImageSource;
     * }} options
     */
    ...
    {
        return this.#Device.features.has("bgra8unorm-storage") && Device.PreferredCanvasFormat || "rgba8unorm";
    }
