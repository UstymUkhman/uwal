...
export class Node2D
{
    ...
    GetWorldPosition(update = false)
    {
        ...
        return mat3.getTranslation(this.#WorldMatrix, this.#WorldPosition);
    }

    /** @param {[Transformation, number | undefined, Transformation | number]} transform */
    set Transform(transform)
    {
        ...
        for (let t = 0, tl = transformations.length; t < tl; t++)
            switch(transformations[t])
            {
                ...
                case "S":
                    if (scaling) this.Scaling = /** @type {Vec2} */ (scaling);
                break;
            }
    }
    ...
    /** @param {Vec2 | number} scaling */
    set Scaling(scaling)
    {
        scaling = /** @type {Vec2} */ (typeof scaling === "number" && [scaling, scaling] || scaling);
        vec2.copy(new Float32Array(scaling), this.#Scaling);
        mat3.scale(this.#LocalMatrix, this.#Scaling, this.#LocalMatrix);
    }
