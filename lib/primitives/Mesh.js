...
import { GetMaxAxisScale } from "#/utils/Math";
...
{
    /**
     * @typedef {import("../pipelines/Pipeline").Immediate} Immediate
     * @typedef {import("../geometries").MeshGeometry} Geometry
     * @import {Mat3, Mat4} from "wgpu-matrix"
     */
    ...
    /** @type {Immediate | undefined} */ #Immediates;
    ...
    /** @type {import("../pipelines/Pipeline").BindGroup[]} */ BindGroups = [];
     ...
     * @param {GPUBindingResource | GPUBindingResource[]} resources
     * @param {number | number[]} bindings
     * @param {import("../pipelines/Pipeline").BufferDescriptor} [descriptor]
     */
    #CreateMatrixBuffer(resources, bindings, descriptor)
    {
        ...
        resources = GetParamArray(resources).filter(Boolean);
        resources.unshift(buffer);

        bindings = GetParamArray(bindings).filter(Number.isFinite);
        bindings.unshift(BINDINGS.MESH_MATRIX);

        !bindings.includes(BINDINGS.CAMERA_MATRIX) &&
            ThrowWarning(ERROR.CAMERA_BINDING_NOT_FOUND, `\`Mesh.SetRenderPipeline\`.
                Camera's \`MatrixBuffer\` is required at binding "${BINDINGS.CAMERA_MATRIX}" in the vertex shader.`
            );
    ...
    UpdateMatrixBuffer()
    {
        ...
        // Transpose and invert the world matrix into a normal matrix for light calculations:
        mat3.fromMat4(mat4.inverse(mat4.transpose(WorldMatrix)), this.#NormalMatrix);

        /** @type {RenderPipeline} */ (this.Pipeline).WriteBuffer(
            /** @type {GPUBuffer} */ (this.#MatrixBuffer),
            /** @type {BufferSource} */ (this.#WorldMatrix.buffer)
        );
    }
    ...
    /** @param {Immediate | GPUAllowSharedBufferSource | undefined} immediates */
    set Immediates(immediates)
    {
        this.#Immediates =
            typeof immediates !== "undefined" && typeof /** @type {Immediate} */ (immediates).rangeOffset !== "number"
            ? { data: /** @type {GPUAllowSharedBufferSource} */ (immediates), rangeOffset: 0 }
            : /** @type {Immediate | undefined} */ (immediates);
    }

    /** @returns {Immediate | undefined} */
    get Immediates()
    {
    ...
    get Radius()
    {
        return this.#Geometry.Radius * GetMaxAxisScale(this.WorldMatrix);
    }
