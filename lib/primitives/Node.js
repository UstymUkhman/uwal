...
export class Node
{
    ...
    /** @type {Vec3} */ #WorldPosition = vec3.create();
    ...
    UpdateWorldMatrix()
    {
        ...
        // `this.#Parent` is always defined and at least is a `Scene` instance in order for a `Node` to be updated:
        mat4.multiply(/** @type {Scene | Node} */ (this.#Parent).WorldMatrix, this.#LocalMatrix, this.#WorldMatrix);
    ...
    GetWorldPosition(update = false)
    {
        ...
        return mat4.getTranslation(this.#WorldMatrix, this.#WorldPosition);
    }

    /** @param {[Transformation, Transformation, Transformation | number]} transform */
    set Transform(transform)
    {
        ...
        for (let t = 0, tl = transformations.length; t < tl; t++)
            switch(transformations[t])
            {
                ...
                case "S":
                    if (scaling) this.Scaling = /** @type {Vec3} */ (scaling);
                break;
            }
    }
    ...
    /** @param {Vec3 | number} scaling */
    set Scaling(scaling)
    {
        scaling = /** @type {Vec3} */ (typeof scaling === "number" && [scaling, scaling, scaling] || scaling);
        vec3.copy(new Float32Array(scaling), this.#Scaling);
        mat4.scale(this.#LocalMatrix, this.#Scaling, this.#LocalMatrix);
    }
