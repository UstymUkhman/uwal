import { CreateConstantObject, SetDrawParams } from "#/utils";
...
export default class Shape
{
    ...
    #CreatePositionBuffer(Pipeline, label = this.#Descriptor.label)
    {
        ...
        if (!vertexData)
        {
            ...
            vertexData = new Float32Array((this.#Segments + 1) * 4);

            const { endAngle = TAU, startAngle = 0, innerRadius = 0 } = this.#Descriptor;
    ...
    #CreateIndexBuffer(Pipeline, label = this.#Descriptor.label)
    {
        ...
        if (!indexData)
        {
            ...
            for (let index = 0, offset = 0, i = 0, l = this.#Segments; i < l; ++i, offset = i * 2)
            {
    ...
    /**
     * @param {RenderPipeline} Pipeline
     * @param {string} [vertexEntry = ""]
     */
    #CreateNormalBuffer(Pipeline, vertexEntry = "")
    {
        if (!this.#AddNormalBuffer || this.#VertexData) return;

        const normals = new Float32Array((this.#Segments + 1) * 6);

        for (let s = 0, l = this.#Segments; s <= l; ++s) normals.set([0, 0, 1, 0, 0, 1], s * 6);

        this.AddNormalBuffer(Pipeline, normals, vertexEntry || "vertexNormal");
    }
     ...
     * @param {string} [vertexEntry = ""]
     */
    #CreateUVBuffer(Pipeline, vertexEntry = "")
    {
        if (!this.#AddUVBuffer || this.#VertexData) return;

        const { endAngle = TAU, startAngle = 0 } = this.#Descriptor;
        ...
        this.AddUVBuffer(Pipeline, uvs, vertexEntry || "vertexUV");
    }
    ...
    CreateBuffers(Pipeline, label = this.#Descriptor.label)
    {
        ...
        if (!this.#AddNormalBuffer) return this.#CreateUVBuffer(Pipeline, vertexEntry);

        this.#CreateNormalBuffer(Pipeline, vertexEntry);
        this.#CreateUVBuffer(Pipeline, vertexEntry);
    }

    /**
     * @param {RenderPipeline} Pipeline
     * @param {import("../pipelines/Pipeline").TypedArray<ArrayBuffer>} data
