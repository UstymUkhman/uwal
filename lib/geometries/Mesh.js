...
/**
 * @typedef {import("../pipelines/Pipeline").TypedArray<ArrayBuffer>} VertexData
 ...
 *
 * @typedef {Object} PrimitiveGeometry
 * @property {Float32Array} positions
 * @property {VertexData} normals
 * @property {VertexData} uvs
 * @property {IndexArray<ArrayBufferLike>} cells
 */
...
export default class Mesh
{
    ...
    * @property {keyof typeof Primitives} name
    * @property {PrimitiveGeometry} [geom]
    * @property {string} [vertexEntry]
    * @property {boolean} [normals]
    * @property {boolean} [uvs]
    ...
    */
    ...
    /** @type {Primitive} */ #Primitive = /** @type {Primitive} */ ({});
    ...
    /**
     * @param {Primitive | keyof typeof Primitives} [primitive]
     * @param {GPUIndexFormat} [format]
     * @param {string} [label = "Mesh"]
     */
    constructor(primitive, format, label = "Mesh")
    {
        this.#ID = ID++;
        this.#Label = label;
        this.IndexFormat = format;
        utils.setTypedArrayType(format === "uint16" && Uint16Array || Uint32Array);
        if (primitive) this.#Primitive = typeof primitive === "object" && primitive || { name: primitive };
    }
    ...
    {
        if (!this.#Primitive.name) return;

        const { cells, positions } = this.#Primitive.geom ??=
            /** @type {(options?: Object) => PrimitiveGeometry} */
            (Primitives[this.#Primitive.name])(this.#Primitive.args);

        this.IndexFormat = cells instanceof Uint16Array && "uint16" || "uint32";
        this.CreatePositionBuffer(Pipeline, positions, label);
        this.CreateIndexBuffer(Pipeline, cells, label);

        const { normals: normal, uvs: uv } = this.#Primitive;
        const { normals, uvs } = this.#Primitive.geom;

        const vertexEntry = this.#Primitive.vertexEntry
            || normal && uv && "vertexNormalUV"
            || normal && "vertexNormal"
            || "vertexUV";

        normal && normals && this.AddNormalBuffer(Pipeline, normals, vertexEntry);
        uv && uvs && this.AddUVBuffer(Pipeline, uvs, vertexEntry);
    }
     ...
     * @param {RenderPipeline} Pipeline
     * @param {IndexArray<ArrayBufferLike>} indexData
     * @param {number} [size = this.#PositionAttributeSize]
     * @param {string} [label = this.#Label]
     */
    CreateEdgeBuffer(Pipeline, indexData, size = this.#PositionAttributeSize, label = this.#Label)
    {
        const { length } = indexData, uint16IndexData = indexData instanceof Uint16Array;
        const edgeData = new (uint16IndexData && Uint16Array || Uint32Array)(length * 2);
        this.IndexFormat = uint16IndexData && "uint16" || "uint32";

        for (let i = 0, cell = 0; i < length; i += size)
        {
            for (let s = 0; s < size; cell += 2)
            {
                const i0 = indexData[i + s];
                const i1 = indexData[i + ++s % size];

                edgeData[cell + 0] = Math.min(i0, i1);
                edgeData[cell + 1] = Math.max(i0, i1);
            }
        }

        this.CreateIndexBuffer(Pipeline, edgeData, label);
    }
     ...
     * @param {RenderPipeline} Pipeline
     * @param {VertexData} data
     * @param {string} [vertexEntry = "vertexNormal"]
     * @param {VertexAttribute | VertexAttribute[]} [attributes = "normal"]
     * @param {GPUVertexStepMode} [stepMode = "vertex"]
     */
    AddNormalBuffer(Pipeline, data, vertexEntry = "vertexNormal", attributes = "normal", stepMode = "vertex")
    {
    ...
    GetPositionBufferLayout(
        ...
        const layout = Pipeline.CreateVertexBufferLayout(attributes, vertexEntry);
     ...
     * @param {GPUVertexStepMode} [stepMode = "vertex"]
     */
    GetNormalBufferLayout(Pipeline, vertexEntry = "vertexNormal", attributes = "normal", stepMode = "vertex")
    {
        return (this.#Primitive.normals = true) && Pipeline.CreateVertexBufferLayout(attributes, vertexEntry, stepMode);
    }
    ...
    {
        return (this.#Primitive.uvs = true) && Pipeline.CreateVertexBufferLayout(attributes, vertexEntry, stepMode);
    }

    /**
     * @param {RenderPipeline} Pipeline
     * @param {string} [vertexEntry = "vertex"]
     * @param {VertexAttribute[]} [attributes = ["instanceColumn0", "instanceColumn1", "instanceColumn2", "instanceColumn3"]]
     */
    GetInstanceBufferLayout(
        Pipeline,
        vertexEntry = "vertex",
        attributes = ["instanceColumn0", "instanceColumn1", "instanceColumn2", "instanceColumn3"]
    ) {
    ...
    /** @returns {PrimitiveGeometry | undefined} */
    get Primitive()
    {
        return this.#Primitive.geom;
    }
