...
export default class Mesh
{
    ...
    CreateBuffers(Pipeline, label = this.#Label)
    {
        ...
        this.#AddNormalBuffer && this.AddNormalBuffer(
            Pipeline, /** @type {VertexData} */ (this.Primitive.normals), vertexEntry
        );

        this.#AddUVBuffer && this.AddUVBuffer(
            Pipeline, /** @type {VertexData} */ (this.Primitive.uvs), vertexEntry
        );
    }
     ...
     * @param {string} [label = this.#Label]
     */
    CreateEdgeBuffer(Pipeline, size = this.#PositionAttributeSize, indexData, label = this.#Label)
    {
        indexData ??= /** @type {IndexArray<ArrayBufferLike>} */ (this.Primitive?.cells);

        !indexData && ThrowError(ERROR.INDEX_DATA_NOT_FOUND, `\`CreateEdgeBuffer\` method.
           If \`Primitive\` geometry is not set, \`indexData\` argument is required in this method call.`
        );
    ...
    GetInstanceBufferLayout(
        ...
        return attributes.map(attribute => Pipeline.CreateVertexBufferLayout(attribute, vertexEntry, "instance"));
    }
