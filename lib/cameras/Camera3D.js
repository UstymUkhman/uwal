...
export class Camera3D extends Node
{
    ...
    #UpdateViewBuffer()
    {
        this.#Pipeline?.WriteBuffer(
            ...
            /** @type {GPUAllowSharedBufferSource} */ (this.#ViewMatrix)
        );
    }
    ...
    #UpdateWorldMatrixBuffer()
    {
        this.#Pipeline?.WriteBuffer(
            /** @type {GPUBuffer} */ (this.#MatrixBuffer),
            /** @type {GPUAllowSharedBufferSource} */ (this.WorldMatrix), 64
        );
    }
    ...
    #UpdateViewProjectionBuffer()
    {
        this.#Pipeline?.WriteBuffer(
            /** @type {GPUBuffer} */ (this.#MatrixBuffer),
            /** @type {GPUAllowSharedBufferSource} */ (this.#ViewProjectionMatrix), 192
        );
    }
    ...
    UpdateProjectionMatrix()
    {
        this.#Pipeline?.WriteBuffer(
            /** @type {GPUBuffer} */ (this.#MatrixBuffer),
            /** @type {GPUAllowSharedBufferSource} */ (this.ProjectionMatrix), 128
        );
    }
     ...
     * @see [SkyBox](https://ustymukhman.github.io/uwal/dist/lessons/lessons.html#skybox) for reference.
     *
     * @param {boolean} [update] - Whether to update the `ViewProjectionMatrix` before inverting it.
     ...
     */
    GetInverseViewProjectionMatrix(update, translation, dst = mat4.identity())
    {
        update && this.UpdateViewProjectionMatrix();
