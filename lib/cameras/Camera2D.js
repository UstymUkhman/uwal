...
export default class Camera2D extends Node2D
{
    ...
    Contains(Shape)
    {
        ...
        {
            const { Center, Scaling, Geometry } = Shape;

            const x = Center[0] - MathUtils.Clamp(Center[0], l, r);
            const y = Center[1] - MathUtils.Clamp(Center[1], t, b);

            const radius = Math.max(...Scaling) * Geometry.Radius;
