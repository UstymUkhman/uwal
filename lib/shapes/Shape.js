...
export default class Shape
{
    ...
    #CreateUniformBuffer()
    {
        const { color, matrix } = /** @type {Record<String, Float32Array>} */ (
            /** @type {import("../pipelines/BasePipeline").UniformLayout} */ (
                this.#Renderer.CreateUniformBufferLayout("shape")
            )
        );

        this.#Color = color; this.#Matrix = matrix;

        this.#UniformBuffer = this.#Renderer.CreateBuffer(
        {
            size: (color.length + matrix.length) * Float32Array.BYTES_PER_ELEMENT,
