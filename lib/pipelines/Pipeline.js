import { WgslReflect } from "#/reflect/wgsl_reflect.module.dev";
import { ERROR, ThrowWarning, ThrowError } from "#/Errors";
import { Empty as EmptyShader } from "#/shaders";
import { BUFFER } from "#/pipelines/Constants";
 ...
 *
 * @typedef {Object} Immediate
 * @property {GPUAllowSharedBufferSource} data
 * @property {GPUSize32} rangeOffset
 * @property {GPUSize64} [dataOffset]
 * @property {GPUSize64} [dataSize]
 *
 ...
 *
 * @typedef {Pick<Partial<GPUBufferDescriptor>, "usage"> & Omit<GPUBufferDescriptor, "usage">} SizeDescriptor
 * @typedef {Pick<Partial<SizeDescriptor>, "size"> & Omit<SizeDescriptor, "size"> } BufferDescriptor
 *
 * @typedef {Pick<Partial<GPUBindGroupLayoutEntry>, "binding"> &
 *     Omit<GPUBindGroupLayoutEntry, "binding">
 * } BindGroupLayoutEntry
 *
 * @exports ShaderCode, BindGroup, Immediate, ShaderModuleDescriptor
 * @exports PipelineDescriptor, BufferDescriptor, BindGroupLayoutEntry
 */
...
export default /** @abstract */ class Pipeline
{
    ...
    /** @type {Immediate[]} */ Immediates = [];

    /** @protected @type {WgslReflect | undefined} */ Reflect;
    /** @type {GPURenderPipeline | GPUComputePipeline | undefined} */ GPUPipeline;

    /**
     * @import {MemberInfo, TemplateInfo, StructInfo, VariableInfo} from "../reflect/src/reflect/info"
     ...
     * @param {string} [label = ""]
     */
    constructor(ID, device, type, label = "")
    {
        this.#ID = ID;
        this.#Type = type;
        this.Active = true;
    ...
    /**
     * @typedef {GPUBindGroupLayout | null} BindGroupLayout
    ...
    CreateShaderModule(shader, label, hints)
    {
        ...
        const code = GetParamArray(shader).join("\n\n");
    ...
    {
        const { format } = /** @type {TemplateInfo} */ (member.type);

        const members = /** @type {StructInfo} */ (member.type).members ??
            // `format` can have `members` array only when it's an array of struct:
            /** @type {StructInfo} */ (format)?.members;
    ...
    {
        return this.Device.createBuffer({ label: /*@__INLINE__*/ this.CreatePipelineLabel("Buffer"), ...descriptor });
    }

    /** @param {SizeDescriptor | GPUSize64} sizeOrDescriptor */
    CreateReadableBuffer(sizeOrDescriptor)
    {
        const sizeDescriptor = typeof sizeOrDescriptor === "number";
        const size = +(sizeDescriptor && sizeOrDescriptor);

        const usage = BUFFER.READABLE | (!sizeDescriptor && sizeOrDescriptor.usage || 0);
        return this.CreateBuffer({ label: "Readable Buffer", size, .../** @type {{}} */ (sizeOrDescriptor), usage });
    }
    ...
    CreateWritableBuffer(sizeOrDescriptor)
    {
        ...
        const usage = BUFFER.WRITABLE | (!sizeDescriptor && sizeOrDescriptor.usage || 0);
        return this.CreateBuffer({ label: "Writable Buffer", size, .../** @type {{}} */ (sizeOrDescriptor), usage });
    }
    ...
    CreateUniformBuffer(uniformName, descriptor)
    {
        ...
        const label = `${uniformName} Uniform Buffer`, usage = BUFFER.UNIFORM | (descriptor?.usage || 0);
    ...
    CreateStorageBuffer(storageName, descriptor = 1)
    {
        ...
        const usage = BUFFER.STORAGE | (typeof descriptor !== "number" && descriptor.usage || 0);
        const length = typeof descriptor === "number" ? descriptor : descriptor.length || 0;

        const size = /** @type {BufferDescriptor} */ (descriptor).size ??
            (/** @type {VariableInfo} */ (storage).format?.size || 0) * length;

        const label = `${storageName} Storage Buffer`;
        ...
        const layout = this.#CreateMemberLayout(/** @type {MemberInfo} */
    ...
    /**
     * @template {string} LayoutName
     * @param {{ [Name in LayoutName]: Layout } & { buffer: GPUBuffer }} buffer
     * @param {ArrayLike<number>} data
     * @returns {GPUBuffer}
     */
    WriteBufferData(buffer, data)
    {
        const { buffer: Buffer, ...layout } = buffer;
        const Layout = Object.values(layout)[0];

        Layout.set(data);
        return this.WriteBuffer(Buffer, Layout);
    }
    ...
    {
        const last = /** @type {GPUIndex32[]} */ (bindings)?.length - 1;

        /** @param {number} binding */
        const GetBinding = (binding) => /** @type {GPUIndex32[]} */ (bindings)[last] !== void 0
            ? /** @type {GPUIndex32[]} */ (bindings)[last] + binding - last
            : /** @type {number} */ (bindings) + binding;
        ...
        return /** @type {GPUBindGroupEntry[]} */ (
            ...
            || /** @type {GPUBindingResource[]} */ (resources).map((resource, binding) => ({
                binding: /** @type {GPUIndex32[]} */ (bindings)[binding] ?? /*@__INLINE__*/ GetBinding(binding),
                resource
            }))
        );
    }
    ...
    CreateBindGroup(entries, layout = 0, label)
    {
        ...
        {
            const stage = this.#Type === "Compute" && "Computation" || `${this.#Type}er`;
            ...
            layout = /** @type {GPUBindGroupLayout} */ (this.GPUPipeline
                ...
                : ThrowError(ERROR.PIPELINE_NOT_FOUND, `${this.#Type}Pipeline.
                    Use \`${stage}.AddPipeline\` before creating a bind group.`
                )
            );
        }
     ...
     * @param {GPUBindGroupLayout | number} [layout]
     * @param {GPUBufferDynamicOffset | GPUBufferDynamicOffset[] | GPUBufferDynamicOffset[][]} [dynamicOffsets]
     */
    SetBindGroups(bindGroups, layout, dynamicOffsets)
    {
        ...
        if (layout && typeof layout === "number" && !groupsArray)
            this.BindGroups[layout] = { bindGroup: bindGroups, dynamicOffsets, active: true };
     ...
     * @param {GPUBufferDynamicOffset | GPUBufferDynamicOffset[] | GPUBufferDynamicOffset[][]} [dynamicOffsets]
     */
    AddBindGroups(bindGroups, layout, dynamicOffsets)
    {
        const groupsArray = Array.isArray(bindGroups);
        const offsetsArray = Array.isArray(dynamicOffsets);
        ...
        if (layout && typeof layout === "number" && !groupsArray)
            ...
            !groupsArray &&
                this.BindGroups.push({ bindGroup: bindGroups, dynamicOffsets, active: true }) ||
                this.BindGroups.push.apply(this.BindGroups, /** @type {GPUBindGroup[]} */ (bindGroups).map(
                    (bindGroup) => ({ bindGroup, dynamicOffsets, active: true })
                ));
    ...
    /**
     * @param {Immediate} [immediates]
     */
    SetImmediates(immediates)
    {
        this.Immediates = immediates && [immediates] || [];
    }

    /**
     * @param {GPUAllowSharedBufferSource} data
     * @param {GPUSize32} [rangeOffset = 0]
     * @param {GPUSize64} [dataOffset]
     * @param {GPUSize64} [dataSize]
     */
    AddImmediates(data, rangeOffset = 0, dataOffset, dataSize)
    {
        this.Immediates.push({ data, rangeOffset, dataOffset, dataSize });
    }
