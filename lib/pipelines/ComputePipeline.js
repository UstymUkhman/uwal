...
export default class ComputePipeline extends BasePipeline
{
    ...
    Compute()
    {
        const pass = this.GetCommandEncoder().beginComputePass(this.Descriptor);

        pass.setPipeline(/** @type {GPUComputePipeline} */ (this.Pipeline));

        for (let g = 0, l = this.BindGroups.length; g < l; ++g)
            pass.setBindGroup(g, this.BindGroups[g].bindGroup, this.BindGroups[g].dynamicOffsets);
