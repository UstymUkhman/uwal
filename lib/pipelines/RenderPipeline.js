 ...
 * @typedef {import("./Pipeline").PipelineDescriptor & RenderPipelineState} RenderPipelineDescriptor
 * @typedef {Omit<import("./Pipeline").default & RenderPipeline, "Init">} RenderPipelineInstance
 * @typedef {{ name: string; format?: GPUVertexFormat } | string} VertexAttribute
 *
 * @typedef {Object} RenderPipelineState
 ...
 * @property {GPUSize64} [size]
 *
 * @exports DrawParams, RenderPipelineDescriptor, VertexAttribute
 * @exports RenderPipelineInstance, VertexBuffer, IndexBufferParams
 */
...
export default class RenderPipeline extends Pipeline
{
     ...
     * @property {string | number} location
     * @property {{ size: number } | null} type
     *
     * @typedef {import("./Pipeline").BindGroup} BindGroup
     * @typedef {import("../utils/Color").ColorParam} ColorParam
     * @typedef {import("./Pipeline").BufferDescriptor} BufferDescriptor
    ...
    /** @type {GPUTexture | GPUTextureView | undefined} */ TextureView;
    /** @type {GPUColor} */ #BlendConstant = new Array(4).fill(0);
     ...
     * @param {string} [label = ""]
     */
    constructor(device, format, label = "")
    {
        super(ID++, device, "Render", label);
    ...
    async Init(moduleDescriptor = {})
    {
        ...
        this.#Transparent = !!(fragment?.targets) && fragment.targets.some(target => target?.blend);
     ...
     * @param {Record<string, GPUPipelineConstantValue | boolean>} [constants]
    ...
    CreateVertexState(module, entryPoint = "vertex", buffers, constants)
    {
        ...
        return /** @type {GPUVertexState} */ ({ module, entryPoint, buffers, constants });
    }
    ...
    CreateFragmentState(module, entryPoint = "fragment", targets, constants)
    {
        ...
        return /** @type {GPUFragmentState} */ ({ module, entryPoint, targets, constants });
    }
    ...
    CreateVertexBufferLayout(attributes, vertexEntry = "vertex", stepMode = "vertex")
    {
        ...
        const { entry: { vertex } } = /** @type {import("#/WGSLReflect").WgslReflect} */ (this.Reflect);
        ...
        for (let a = 0, l = attributes.length; a < l; ++a)
        {
            ...
            const input = /** @type {{ inputs: InputInfo[] }} */
    ...
    CreateVertexBuffer(dataOrAttributes, descriptor = 1, vertexEntry = "vertex", stepMode = "vertex")
    {
        ...
        const usage = BUFFER.VERTEX | (!countDescriptor && descriptor.usage || 0);
    ...
    AddVertexBuffers(vertexBuffers, offsets, sizes)
    {
        ...
        !Array.isArray(vertexBuffers) &&
            this.VertexBuffers.push({ buffer: vertexBuffers, offset: offsets[0], size: sizes[0] }) ||
            this.VertexBuffers.push.apply(this.VertexBuffers, /** @type {GPUBuffer[]} */ (vertexBuffers).map(
                (buffer, b) => ({ buffer, offset: offsets[b], size: sizes[b] })
            ));
    ...
    CreateIndexBuffer(indices, descriptor)
    {
        ...
        const usage = BUFFER.INDEX | (descriptor?.usage || 0);
    ...
    EncodeRenderBundle(encoder)
    {
        ...
        encoder[this.DrawMethod].apply(encoder, this.DrawParams);
     ...
     * @param {number | DrawParams} [drawParams = this.DrawParams]
     * @param {VertexBuffer | VertexBuffer[]} [vertexBuffers]
     * @param {IndexBufferParams} [indexBuffer]
     * @param {BindGroup | BindGroup[]} [bindGroups]
     */
    SetRenderBundle(encoder, drawMethod, drawParams = this.DrawParams, vertexBuffers, indexBuffer, bindGroups)
    {
     ...
     * @param {BindGroup | BindGroup[]} [bindGroups]
     */
    AddRenderBundle(encoder, drawMethod, drawParams = this.DrawParams, vertexBuffers, indexBuffer, bindGroups)
    {
        encoder.setPipeline(/** @type {GPURenderPipeline} */ (this.GPUPipeline));
        ...
        encoder[drawMethod].apply(encoder, /** @type {DrawParams} */ (GetParamArray(drawParams)));
