 ...
 *
 * @typedef {
       Pick<Partial<GPUBufferDescriptor>, "usage" | "size"> &
       Omit<GPUBufferDescriptor, "usage" | "size">
   } BufferDescriptor
 *
 * @exports ShaderModuleDescriptor, BasePipelineDescriptor, BufferDescriptor, ShaderCode, BindGroup
 */
...
/** @abstract */ export default class BasePipeline
{
    ...
    #CreateMemberLayout(member, buffer, lastOffset = 0, views = [])
    {
        ...
        {
            const type = /*@__INLINE__*/ GetBaseType((format ?? member.type).name);
            const length = member.size / /*@__INLINE__*/ GetTypeBytes(type);
            const TypedArray = /*@__INLINE__*/ GetTypedArray(type);
    ...
    CreateStorageBuffer(storageName, descriptor = 1)
    {
        ...
        const byteLength = /** @type {BufferDescriptor} */ (descriptor).size ??
    ...
    {
        WriteBuffer(this.Device.queue, buffer, data, bufferOffset, dataOffset, size);
    }
    ...
    AddBindGroups(bindGroups, dynamicOffsets)
    {
        ...
        this.BindGroups.push(...(groupsArray
