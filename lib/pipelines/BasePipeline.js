...
/** @abstract */ export default class BasePipeline
{
    ...
    /**
     * @typedef {Pick<Partial<GPUBufferDescriptor>, "usage"> & Omit<GPUBufferDescriptor, "usage">} BufferDescriptor
     * @param {GPUSize64 | BufferDescriptor} sizeOrDescriptor
     */
    CreateReadableBuffer(sizeOrDescriptor)
    {
        let size = typeof sizeOrDescriptor === "number" && sizeOrDescriptor;
        size ||= /** @type {BufferDescriptor} */ (sizeOrDescriptor).size;
        const label = sizeOrDescriptor?.label ?? "Readable Buffer";
        return this.CreateBuffer({ size, usage: USAGE.READABLE, label, ...sizeOrDescriptor });
    }
    ...
    CreateWritableBuffer(sizeOrDescriptor)
    {
        ...
        return this.CreateBuffer({ size, usage: USAGE.WRITABLE, label, ...sizeOrDescriptor });
    }
     ...
     * @param {number | BufferDescriptor & { entries: number }} [descriptor]
    ...
    CreateStorageBuffer(storageName, descriptor = 1)
    {
        ...
        const entries = (typeof descriptor === "number" && descriptor) || descriptor.entries;
        const label = descriptor.label ?? `${storageName} Storage Buffer`;

        const byteLength = entries * storage.format.size;
        ...
        let layout = this.#CreateMemberLayout(/** @type import("wgsl_reflect").MemberInfo */
            (/** @type {unknown} */ storage), arrayBuffer
        );

        layout = layout.buffer instanceof ArrayBuffer
            ? new layout.constructor(arrayBuffer, 0, entries) : CreateTypedArrays(layout);

        return {
            buffer: this.CreateBuffer({ size: byteLength, usage: USAGE.STORAGE, label, ...descriptor }),
            [storageName]: layout
        };
    }
    ...
    CreateUniformBuffer(uniformName, descriptor)
    {
        ...
        return {
            buffer: this.CreateBuffer({ size: arrayBuffer.byteLength, usage: USAGE.UNIFORM, label, ...descriptor }),
    ...
    CreateUniformBufferLayout(uniformName)
    {
        ...
        return this.#CreateMemberLayout(
