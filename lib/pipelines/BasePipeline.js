...
import { GetTypeFormat, GetFormatBytes } from "@/Utils";
...
/** @abstract */ export default class BasePipeline
{
     ...
     * @param {Object[]} [views = []]
     */
    #GetMemberViews(member, offset = 0, views = [])
    {
        ...
        let byteOffset = offset + (member.offset ?? 0);

        if (members)
            // Create a new struct for each element of `format.members` array:
            for (let c = 0, v = {}, l = (format?.isStruct && member.count || 1); c < l; ++c)
            {
                members.forEach(member => v[member.name] = this.#GetMemberViews(member, byteOffset));
                format?.isStruct && (byteOffset += member.stride);
                views.push(v);
            }
        else
        {
            const type = GetTypeFormat((format ?? member.type).name);
            const length = member.size / GetFormatBytes(type);
            return { type, byteOffset, length };
        }
    ...
    CreateUniformBufferLayout(uniformName)
    {
        ...
        this.#GetMemberViews(/** @type import("wgsl_reflect").MemberInfo */ (/** @type {unknown} */ (uniform)));
    }
