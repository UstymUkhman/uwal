...
/** @abstract */ export default class BasePipeline
{
    ...
    /** @param {import("wgsl_reflect").VariableInfo | import("wgsl_reflect").MemberInfo} member */
    #GetStructMemberType({ type })
    {
        const { format } = /** @type {import("wgsl_reflect").TemplateInfo} */ (type);

        const members = /** @type {import("wgsl_reflect").StructInfo} */ (type).members ??
        ...
        if (members)
        {
            ...
            for (let m = 0; m < (format?.isStruct && members.length - 1 || 1); ++m)
                members.forEach(this.#GetStructMemberType.bind(this));
        }
        else
        {
            GetTypeSize((format ?? type).name);
        }
    }
    ...
    {
        // !this.Reflect && ThrowError(ERROR.SHADER_MODULE_NOT_FOUND);
        ...
        this.#GetStructMemberType(uniform);
    }
