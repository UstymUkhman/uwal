...
export default class RenderStage extends BaseStage
{
    ...
    /**
     * @param {Scene | boolean} [sceneOrSubmit = true] - When a boolean, complete recording of the render pass commands
     * sequence (`GPURenderPassEncoder.end()`), complete recording of the commands sequence (`GPUCommandEncoder.finish()`),
     ...
     */
    Render(sceneOrSubmit = true)
    {
        if (sceneOrSubmit instanceof Scene) return this.#RenderScene(sceneOrSubmit);
        ...
        if (!(pl - 1) && this.Pipelines[0].Active)
        {
            ...
            this.#UsePipeline(this.Pipelines[0], sceneOrSubmit, false);
        }
        else
        {
            for (let p = 0; p < pl; ++p)
            {
                ...
                Pipeline.Active && this.#UsePipeline(Pipeline, sceneOrSubmit, true);
            }
        }

        sceneOrSubmit && this.Submit();
    }
