...
import { GetParamArray, GetShaderModule, GetGPUColorValue } from "#/utils";

/**
 * @typedef {import("../pipelines/RenderPipeline").RenderPipelineDescriptor & {
 *     useInRenderPass?: boolean;
 * }} RenderPipelineDescriptor
 *
 * @exports RenderPipelineDescriptor
 */

export default class RenderStage extends Stage
{
    /**
     * @typedef {import("../pipelines/Pipeline").ShaderCode} ShaderCode
     ...
     *
     * When `useInRenderPass` is `true`, call immediately `GPURenderCommandsMixin.setPipeline()` with
     * pipeline created from this descriptor to use it in an already created `GPURenderPassEncoder`.
     * Throws a `RENDER_PASS_NOT_FOUND` warning if there's no such active and recording `GPURenderPassEncoder`.
     *
     * @typedef {ShaderCode               |
     *           GPUShaderModule          |
     ...
     * @param {string} [label = ""]
     */
    constructor(device, canvas, options, label = "")
    {
        super(device, "Render", label);
    ...
    #UpdateDepthStencilAttachment()
    {
        ...
        if (!this.#DepthTexture || this.#DepthTexture.width !== width || this.#DepthTexture.height !== height)
        {
            ...
            {
                sampleCount: this.#MultisampleTexture?.sampleCount,
        ...
        depthStencilAttachment.view = this.#DepthTexture;
    }
     ...
     * @returns {import("../pipelines/Pipeline").BindGroupLayoutEntry}
     */
    ...
    #UpdateCanvasResolution()
    {
        ...
        this.#Resolution.set([width, height, width / height, this.DevicePixelRatio]);
        this.Device.queue.writeBuffer(this.#ResolutionBuffer, 0, this.#Resolution);
    }
    ...
    async CreatePipeline(moduleDescriptor)
    {
        const Pipeline = new this.Pipeline(
            /** @type {RenderPipelineDescriptor & { pipelineLabel?: string }} */ (moduleDescriptor).pipelineLabel
        );
    ...
    async AddPipeline(Pipeline, moduleDescriptor)
    {
        ...
        const useInRenderPass = /** @type {RenderPipelineDescriptor} */ (moduleDescriptor)?.useInRenderPass;
    ...
    #UsePipeline(Pipeline, submit, setPipeline, setRenderBuffers = true, setBlendConstant = setPipeline)
    {
        ...
        if (!this.#RenderPass)
        {
            ...
            if (this.#MultisampleTexture)
            {
                ...
                colorAttachment.view = this.#MultisampleTexture;
            }

            Pipeline.UseTextureView &&
                (colorAttachment[colorAttachmentSubresource] = Pipeline.TextureView || this.CurrentTexture);
        ...
        {
            const immediates = Pipeline.Immediates;
            ...
            for (let i = 0, l = Math.max(immediates.length, 1); i < l; ++i)
            {
                setRenderBuffers && Pipeline.UseRenderBuffers(this.#RenderPass);

                if (immediates[i])
                {
                    const { data, rangeOffset, dataOffset, dataSize } = immediates[i];
                    this.#RenderPass.setImmediates(rangeOffset, data, dataOffset, dataSize);
                }
     ...
     * @param {boolean} submit
     */
    #RenderBatch(Batch, submit)
    {
        const { Pipeline, Geometry, Instances } = Batch;
        ...
        for (let i = 0; i < Instances; setPipeline = setRenderBuffers = false)
        {
            ...
            Pipeline.BindGroups = /*@__INLINE__*/ Batch.GetBindGroups(i);
            Pipeline.SetImmediates(/*@__INLINE__*/ Batch.GetImmediates(i++));
    ...
    {
        const size = Clamp(Meshes.length, 8, 64);

        for (let m = 0, batch = /** @type {Batch | void} */ (void 0), l = Meshes.length; m < l; batch = void 0, ++m)
        {
            const Mesh = Meshes[m];
            Mesh.UpdateMatrixBuffer();

            const { Geometry, Pipeline: P } = Mesh;
            ...
            !(batch = Batches.get(id)) && Batches.set(id, batch = new Batch(
                Pipeline, Geometry, Batches.size, size
            ));

            batch.Add(Mesh, Center, this.#Frame);
        }
    ...
    */
    #RenderScene(Scene, submit = true)
    {
        ...
        if (this.#OpaqueMeshes.length)
        {
            this.#GroupMeshes(Camera, this.#OpaqueBatches, this.#OpaqueMeshes);

            this.#OpaqueBatches.forEach((Batch, id) =>
            {
                (Batch.ResizeInstances(this.#Frame) &&
                    ~this.#RenderBatch(Batch.SortOpaque(), submit))
                        || Batch.Delete && this.#OpaqueBatches.delete(id);

                Batch.ResetInstances();
            });
        }

        if (this.#TransparentMeshes.length)
        {
            ...
            const TransparentKeys = Array.from(this.#TransparentBatches.keys());

            if (this.#TransparentBatches.size > 1)
            {
                // Sort transparent batches by the maximum distance value, from the furthest to the closest.
                const Distances = TransparentBatches.map(Batch => Batch.SortTransparent());

                TransparentBatches
                    .map(({ Index }) => Index)
                    .sort((a, b) => Distances[b] - Distances[a])
                    .forEach((index, i) => TransparentBatches[i] = /** @type {Batch} */ (
                        this.#TransparentBatches.get(TransparentKeys[index])
                    ));
            }

            TransparentBatches.forEach(Batch =>
            {
                (Batch.ResizeInstances(this.#Frame) &&
                    ~this.#RenderBatch(Batch, submit)) || Batch.Delete
                        && this.#TransparentBatches.delete(TransparentKeys[Batch.Index]);
    ...
    /**
     * @param {import("../Scene").Scene | boolean} [sceneOrSubmit = true]
    ...
    Render(sceneOrSubmit = true, submit = true)
    {
        ...
        // @ts-ignore Allow to combine scene and pipeline
        // rendering by resetting the batch state on each iteration.
        this.#BatchState.GeometryID = ~(this.#BatchState.Pipeline = void 0);

        for (let p = 0, l = this.Pipelines.length; p < l; ++p)
            this.#UsePipeline(/** @type {Pipeline} */ (this.Pipelines[p]), submit, true);
    ...
    get Pipeline()
    {
        const { Label, Device } = this;
        const format = this.#PreferredCanvasFormat;

        return /** @type {Pipeline & { new(label?: string): Pipeline }} */ (
            /** @type {unknown} */ (class extends RenderPipeline
            {
                constructor(label = Label)
                {
                    super(Device, format, label);
                }
            })
        );
    }
