...
import { GetGPUColorValue, GetParamArray, GetShaderModule } from "#/utils";
...
export default class RenderStage extends BaseStage
{
    ...
    async CreatePipeline(moduleDescriptor, useInRenderPass)
    {
        ...
        moduleDescriptor = GetShaderModule(moduleDescriptor) ?? Pipeline.CreateShaderModule(...Object.values(
            (Array.isArray(moduleDescriptor) || typeof moduleDescriptor === "string")
    ...
    Render(submit = true)
    {
        ...
        if (!(pl - 1) && this.Pipelines[0].Active)
        {
            ...
            this.#UsePipeline(this.Pipelines[0], submit);
        }
        else
        {
            for (let p = 0; p < pl; ++p)
     ...
     * @returns {RenderPipeline & { new(): RenderPipeline }}
     */
    get Pipeline()
    {
