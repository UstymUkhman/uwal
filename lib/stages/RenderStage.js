...
{
    /**
     * @typedef {Array<Shape | Mesh>} Meshes
     * @typedef {import("../primitives").Mesh} Mesh
     * @typedef {import("../primitives").Shape} Shape
     * @typedef {Batch & { Meshes: Meshes }} MeshBatch
     *
     * @typedef {Batch & { Instances: Instances }} InstanceBatch
     * @typedef {{ Pipeline: Pipeline, Geometry: Geometries }} Batch
     ...
     * @typedef {Array<{ ID: number, Depth: number, Radius: number, BindGroups: BindGroup[] }>} Instances
     *
    ...
    /** @type {Map<string, MeshBatch | InstanceBatch>} */ #OpaqueBatches = new Map();
    /** @type {Map<string, (MeshBatch | InstanceBatch) & { Depth: number }>} */ #TransparentBatches = new Map();
     ...
     * @param {Batch} Batch
     * @param {boolean} submit
     * @param {Meshes | Instances} Meshes
     * @param {Float32Array} [ProjectionMatrix]
     */
    #RenderBatch({ Pipeline, Geometry }, submit, Meshes, ProjectionMatrix)
    {
        ...
        for (let Mesh, m = 0; Mesh = /** @type {Mesh} */ (Meshes[m]); ++m, setPipeline = setRenderBuffers = false)
        {
            ...
            ProjectionMatrix && Mesh.UpdateProjectionMatrix(ProjectionMatrix);
    ...
    #GroupMeshes(Camera, Batches, Meshes)
    {
        ...
        {
            const Mesh = Meshes[m], { Pipeline: P, Geometry, Instances } = Mesh, Pipeline = /** @type {Pipeline} */ (P);
            // Meshes with no Pipeline will be discarded when traversing the scene.
            const id = `${Pipeline.ID}-${Geometry.ID}`;

            !(batch = Batches.get(id)) && Batches.set(id, batch = Object.assign({ Pipeline, Geometry },
                Instances && { Instances: [] } || { Meshes: [] }
            ));

            !Instances
                ? /** @type {MeshBatch} */ (batch).Meshes.push(Mesh)
                : Mesh.UpdateProjectionMatrix(Camera.ViewProjectionMatrix) &&
                /** @type {InstanceBatch} */ (batch).Instances.push(
                {
                    // InstanceMatrix: /** @type {Float32Array} */ (Mesh.InstanceMatrix),
                    // @ts-ignore Correct `Camera` type will treat this argument accordingly.
                    Depth: Camera.GetViewSpaceCenter(Mesh),
                    BindGroups: Mesh.BindGroups,
                    Radius: Mesh.Radius,
                    ID: Mesh.ID
                });
        }
    ...
    */
    #RenderScene(Scene, submit = true)
    {
        ...
        {
            /** @type {InstanceBatch} */ (Batch).Instances?.sort((Prev, Next) =>
                Prev.Depth === Next.Depth && Prev.ID - Next.ID || Prev.Depth - Next.Depth
            ) ||
            /** @type {MeshBatch} */ (Batch).Meshes.sort((Prev, Next) =>
            {
                // @ts-ignore Correct `Camera` type will treat this argument accordingly.
            ...
            /** @type {InstanceBatch} */ (Batch).Instances
                ? this.#RenderBatch(Batch, submit, /** @type {InstanceBatch} */ (Batch).Instances)
                : this.#RenderBatch(Batch, submit, /** @type {MeshBatch} */ (Batch).Meshes, projectionMatrix);
        });
        ...
        if (this.#TransparentBatches.size > 1)
        {
            this.#TransparentBatches.forEach(Batch =>
                /** @type {InstanceBatch} */ (Batch).Instances?.sort((Prev, Next) =>
                    Prev.Depth === Next.Depth && Prev.ID - Next.ID || Next.Depth - Prev.Depth
                )
                ||
                /** @type {MeshBatch} */ (Batch).Meshes.sort((Prev, Next) =>
                {
                ...
                &&
                (Batch.Depth = Math.max.apply(null,
                    /** @type {InstanceBatch} */ (Batch).Instances?.map(({ Depth, Radius }) => Depth + Radius) ||
                    // @ts-ignore Correct `Camera` type will treat this argument accordingly.
                    /** @type {MeshBatch} */ (Batch).Meshes.map(Camera.GetViewSpaceDistance.bind(Camera))
                ))
            );
        ...
        this.#TransparentBatches.forEach(Batch => !/** @type {MeshBatch} */ (Batch).Meshes
            ? this.#RenderBatch(Batch, submit, /** @type {InstanceBatch} */ (Batch).Instances)
            : this.#RenderBatch(Batch, submit, /** @type {MeshBatch} */ (Batch).Meshes, projectionMatrix)
        );
