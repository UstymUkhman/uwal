...
export default class RenderStage extends BaseStage
{
     ...
     * @param {boolean} submit
     */
    #RenderBatch(Batch, Instances, submit)
    {
        const { Pipeline, Geometry } = Batch;
    ...
    */
    #RenderScene(Scene, submit = true)
    {
        ...
        {
            const Instances = /*@__INLINE__*/ Batch.ResetInstances();
            Instances && this.#RenderBatch(Batch.SortOpaque(), Instances, submit)
        });
        ...
        if (this.#TransparentBatches.size > 1)
        {
            ...
            const Keys = Array.from(this.#TransparentBatches.keys());

            TransparentBatches
                .map(({ Index }) => Index)
                .sort((a, b) => Distances[b] - Distances[a])
                .forEach((index, i) => TransparentBatches[i] =
                    /** @type {Batch} */ (this.#TransparentBatches.get(Keys[index]))
                );
        }
        ...
        {
            const Instances = /*@__INLINE__*/ Batch.ResetInstances();
            Instances && this.#RenderBatch(Batch, Instances, submit);
        });
        ...
        /** @todo Add lazy cleanup of unused batches. */
    }
