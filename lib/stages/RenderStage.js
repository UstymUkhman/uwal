...
export default class RenderStage extends BaseStage
{
     ...
     * @typedef {import("../materials").Mesh | undefined} MeshMaterial
     * @typedef {{ Geometry: MeshGeometry, Material: MeshMaterial, Meshes: Mesh[] }} Batch
     *
    ...
    CreateRenderBundleEncoder(
        ...
        label ??= this.CreateStageLabel("Render Bundle Encoder");
    ...
    #RenderScene(Scene, submit = true)
    {
        ...
        let PreviousPipeline = void 0, ppi = -1;
        const opaqueMeshes = [], transparentMeshes = [];
        let PreviousGeometryID = "", PreviousMaterialID = "";
        ...
        Scene.Traverse(node =>
        {
            // Skip the scene and keep traversing.
            if (Scene === node) return false;

            // Collect only visible meshes.
            if (!node.Visible) return true;

            // Discard meshes that have the bounding sphere outside the camera frustum.

            // Separate meshes into 2 lists (opaque and transparent) based on a material property.
            !(node.Material?.Transparent && transparentMeshes || opaqueMeshes).push(node);
        });

        // There are no meshes in the scene, skip rendering.
        if (!opaqueMeshes.length && !opaqueMeshes.length)
            return ~this.DestroyRenderPass() && (this.CommandEncoder = void 0);

        // Opaque objects will be sorted by distance to the camera front to back.
        // Transparent objects will be sorted by distance to the camera back to front.

        // Group objects in each list into batches by geometry and material `id`s.
        for (let m = 0, batch = void 0, l = opaqueMeshes.length; m < l; batch = void 0, ++m)
        {
            const Mesh = opaqueMeshes[m], { Geometry, Material } = Mesh;
            const id = Geometry.ID + `${(Material && `-${Material.ID}`) ?? ''}`;

            if (!this.#OpaqueBatches.has(id))
            {
                batch = { Geometry, Material, Meshes: [] };
                this.#OpaqueBatches.set(id, batch);
            }

            (batch ?? this.#OpaqueBatches.get(id)).Meshes.push(Mesh);
        }

        // Draw each batch objects by setting the geometry only once per batch; switching the material only when
        // its `id` is different form the previous one, and setting material uniforms per every scene object.

        this.#OpaqueBatches.forEach(({ Geometry, Material, Meshes }) =>
        {
            // Switch Material: Material.ID !== PrevMeshMaterialID

            let setRenderBuffers = Geometry.ID !== PreviousGeometryID;

            for (let m = 0, l = Meshes.length; m < l; ppi = PreviousPipeline.Index, setRenderBuffers = false, ++m)
            {
                const Mesh = Meshes[m];
                const Pipeline = Mesh.Pipeline;
                const setPipeline = Pipeline.Index !== ppi;
                ...
                // previous batchs or when the meshes within the same batch have different pipelines.
                if (setPipeline || setRenderBuffers)
                {
                    const { VertexBuffers, IndexBuffer, Vertices } = Geometry;

                    Pipeline.VertexBuffers = VertexBuffers;
                    Pipeline.IndexBuffer = IndexBuffer;
                    Pipeline.SetDrawParams(Vertices);
                ...
                const setBlendConstant = !!Material?.SetBlendConstant(PreviousPipeline);

                // New pipeline will be set only when its index is different from the one setted by the previous mesh.
                this.#UsePipeline(PreviousPipeline = Pipeline, submit, setPipeline, setRenderBuffers, setBlendConstant);
            }
        });
