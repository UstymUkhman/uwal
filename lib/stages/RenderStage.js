...
export default class RenderStage extends BaseStage
{
     ...
     * @typedef {{ Pipeline: Pipeline, Geometry: Geometries, Meshes: Instance[] }} Batch
     ...
     * @typedef {{ ID: number, Depth: number, Far: number, BindGroups: BindGroup[] }} Instance
    ...
    /** @type {Map<string, Batch & { Depth: number }>} */ #TransparentBatches = new Map();
     ...
     * @param {boolean} submit
     */
    #RenderBatch({ Pipeline, Geometry, Meshes }, submit)
    {
    ...
    #GroupMeshes(Camera, Batches, Meshes)
    {
        for (let m = 0, batch = /** @type {Batch | void} */ (void 0), l = Meshes.length; m < l; batch = void 0, ++m)
        {
            ...
            const Depth = Camera.GetViewSpaceCenter(Mesh);
            const Pipeline = /** @type {Pipeline} */ (P);
            const id = `${Pipeline.ID}-${Geometry.ID}`;

            /** @todo Move to structure of Arrays and index-based sorting. */
            !(batch = Batches.get(id)) && Batches.set(id, batch = { Pipeline, Geometry, Meshes: [] });
            batch.Meshes.push({ BindGroups: Mesh.BindGroups, Far: Depth + Mesh.Radius, ID: Mesh.ID, Depth });
        }
    }
    ...
    */
    #RenderScene(Scene, submit = true)
    {
        ...
        this.#OpaqueBatches.forEach(Batch =>
            Batch.Meshes.sort(({ Depth: PrevDepth, ID: PrevID }, { Depth: NextDepth, ID: NextID }) =>
                PrevDepth === NextDepth && PrevID - NextID || PrevDepth - NextDepth
            ) &&
        ...
        if (this.#TransparentBatches.size > 1)
        {
            this.#TransparentBatches.forEach(Batch =>
                Batch.Meshes.sort(({ Depth: PrevDepth, ID: PrevID }, { Depth: NextDepth, ID: NextID }) =>
                    PrevDepth === NextDepth && PrevID - NextID || NextDepth - PrevDepth
                ) &&
                (Batch.Depth = Math.max.apply(null, Batch.Meshes.map(({ Far }) => Far)))
            );
        ...
        this.#TransparentBatches.forEach(Batch => this.#RenderBatch(Batch, submit));
