...
export default class RenderStage extends BaseStage
{
    ...
    #RenderScene(Scene, submit = true)
    {
        ...
        let PreviousPipeline = void 0, ci = -1;
        ...
        {
            const Mesh = opaqueMeshes[m];
            const { Geometry, Material } = Mesh;
            const id = `${Geometry.ID}-${Material.ID}`;
        ...
        this.#OpaqueBatches.forEach(({ Geometry, Material, Meshes }) =>
        {
            ...
            for (
                let m = 0, setRenderBuffers = false, l = Meshes.length; m < l;
                ci = PreviousPipeline.Index, setRenderBuffers = false, ++m
            ) {
                const Mesh = Meshes[m];
                const Pipeline = Mesh.Pipeline;
                const setPipeline = Pipeline.Index !== ci;

                Mesh.UpdateProjectionMatrix(projectionMatrix);

                // The geometry will be set at the beginning of each batch or
                // when meshes within the same batch have different pipelines.
                if (setPipeline || !m)
                {
