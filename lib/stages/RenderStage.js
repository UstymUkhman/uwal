...
export default class RenderStage extends BaseStage
{
     ...
     */
    #UsePipeline(Pipeline, submit, setPipeline, setBlendConstant = true, setRenderBuffers = true)
    {
    ...
    #RenderScene(Scene, submit = true)
    {
        ...
        const { Frustum, ViewProjectionMatrix } = Scene.MainCamera;
        ...
        {
            // Switch Geometry: Geometry.ID !== PrevMeshGeometryID
            // Switch Material: Material.ID !== PrevMeshMaterialID

            for (let m = 0, l = Meshes.length; m < l; ci = PreviousPipeline.Index, ++m)
            {
                const Mesh = Meshes[m];
                const Pipeline = Mesh.Pipeline;

                Mesh.UpdateProjectionMatrix(ViewProjectionMatrix);
                ...
                this.#UsePipeline(Pipeline, submit, (PreviousPipeline = Pipeline).Index !== ci, setBlendConstant);
            }
        });
    ...
    /**
     * @param {Scene | boolean} [sceneOrSubmit = true]
