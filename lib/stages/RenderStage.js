...
export default class RenderStage extends BaseStage
{
    ...
    {
        // Collect only active scene objects by filtering them by the flag `Visible`.
        // Call `UpdateViewProjectionMatrix` on the perspective and `UpdateProjectionMatrix` on the orthographic camera.

        // Extract and normalize 6 planes form the camera's (view) projection matrix [frustum].
        // Discard objects having the sphere bounding box outside the camera's frustum.

        // Sort objects in the scene in 2 lists: opaque and transparent.
        ...
        // Group objects in each list into batches by material and geometry `id`s.
        // Each batch is an entry with material, geometry and a list of objects that share these instances.
        ...
        // Set vertex buffers, index buffer, draw params and bind groups per object,
        // but only when different from those setted by the previous one.

        // Call `#UsePipeline` per object, but pass the `set` flag as `true` only when
        // object's pipeline index is different from the one setted by the previous object.

        // When `set` flag is `true` and a new pipeline will be used, it will also update
        // automatically vertex buffers, index buffer, draw params and bind groups
        // to use those that belong to the newly setted pipeline.
