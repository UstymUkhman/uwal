...
export default class RenderStage extends BaseStage
{
    ...
    #RenderScene(Scene, submit = true)
    {
        ...
        if (!opaqueMeshes.length && !transparentMeshes.length)
        ...
        // Group opaque meshes into batches by geometry and material `id`s.
        ...
        // TODO: Group transparent meshes into batches by geometry and material `id`s.

        this.#OpaqueBatches.forEach(({ Geometry, Material, Meshes }) =>
        {
            let setRenderBuffers = Geometry.ID !== PreviousGeometryID;
            let setBlendConstant = Material?.ID !== PreviousMaterialID;

            for (
                let m = 0, l = Meshes.length; m < l;
                ppi = PreviousPipeline.Index, setRenderBuffers = setBlendConstant = false, ++m)
            {
                const Mesh = Meshes[m];
                ...
                if (setBlendConstant || setPipeline)
                {
                    ...
                    setBlendConstant = !!Material?.SetBlendConstant(PreviousPipeline);
        ...
        // TODO: Render transparent batches.
