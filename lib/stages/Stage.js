 ...
 * @typedef {import("../pipelines/Pipeline").PipelineDescriptor} PipelineDescriptor
 * @typedef {Omit<import("../pipelines/Pipeline").default, "Init">} PipelineInstance
 * @typedef {import("../pipelines/Pipeline").ShaderModuleDescriptor} ShaderModuleDescriptor
 * @typedef {ShaderCode | GPUShaderModule | PipelineDescriptor | ShaderModuleDescriptor} NewPipelineDescriptor
 *
 * @exports NewPipelineDescriptor
 */

export default /** @abstract */ class Stage
{
    /** @typedef {import("../pipelines/Pipeline").BindGroupLayoutEntry} BindGroupLayoutEntry */
    ...
    /** @param {NewPipelineDescriptor} moduleDescriptor */
    async CreatePipeline(moduleDescriptor)
    {
        const { pipelineLabel, shader, label, hints } = /** @type {ShaderModuleDescriptor} */ (moduleDescriptor);
        moduleDescriptor = /** @type {GPUShaderModule | PipelineDescriptor} */ (moduleDescriptor);

        const shaderModuleDescriptor = (moduleDescriptor instanceof GPUShaderModule ||
            Object.hasOwn(moduleDescriptor, "module")) && moduleDescriptor;

        const Pipeline = new this.Pipeline(pipelineLabel);

        return await this.AddPipeline(Pipeline, shaderModuleDescriptor ||
        {
            ...(!Array.isArray(moduleDescriptor) && typeof moduleDescriptor !== "string" && moduleDescriptor),
            module: Pipeline.CreateShaderModule(shader || /** @type {ShaderCode} */ (moduleDescriptor), label, hints)
        });
    }

    /**
     * @param {PipelineInstance} Pipeline
     * @param {GPUShaderModule | PipelineDescriptor} [_moduleDescriptor]
     */
    async AddPipeline(Pipeline, _moduleDescriptor)
    {
    ...
    /** @returns {PipelineInstance & { new(label?: string): PipelineInstance }} */
    get Pipeline()
    {
        return /** @type {Pipeline & { new(label?: string): Pipeline }} */ (/** @type {unknown} */ (Pipeline));
    }
