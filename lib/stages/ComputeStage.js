...
export default class ComputeStage extends Stage
{
    /**
    ...
    /** @type {Workgroups} */ #Workgroups = [8, 8, void 0];
    ...
    async CreatePipeline(moduleDescriptor)
    {
        ...
        const Pipeline = new this.Pipeline(/** @type {ComputePipelineDescriptor} */ (moduleDescriptor).pipelineLabel);
    ...
    async AddPipeline(Pipeline, moduleDescriptor)
    {
        ...
        await /** @type {ComputePipeline} */ (Pipeline).Init(moduleDescriptor);
    ...
    #UsePipeline(Pipeline, computePass)
    {
        ...
        for (let i = 0, l = Math.max(immediates.length, 1); i < l; ++i)
        {
            if (immediates[i])
            {
                ...
                computePass.setImmediates(rangeOffset, data, dataOffset, dataSize);
            }

            computePass.dispatchWorkgroups.apply(computePass, this.#Workgroups);
        }
    ...
    Compute(submit = true)
    {
        ...
        for (let p = 0, l = this.Pipelines.length; p < l; ++p)
            this.#UsePipeline(/** @type {Pipeline} */ (this.Pipelines[p]), computePass);
