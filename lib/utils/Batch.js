export default class Batch
{
    ...
    /**
     * @import {MeshGeometry, ShapeGeometry} from "../geometries"
     * @typedef {MeshGeometry | ShapeGeometry} Geometries
     * @typedef {import("../pipelines/Pipeline").BindGroup} BindGroup
     * @typedef {import("../pipelines/Pipeline").Immediate | undefined} Immediates
    ...
    constructor(Pipeline, Geometry, Index, Size)
    {
        ...
        this.#Immediates = new Array(Size);
    ...
    Add(Mesh, Center, frame)
    {
        ...
        {
            this.#BindGroups.length = this.#Immediates.length = this.#Size = this.#Size << 1;
        ...
        this.#Immediates[i] = Mesh.Immediates;
    ...
    ResizeInstances(frame)
    {
        ...
        {
            this.#BindGroups.length = this.#Immediates.length = this.#Size = Math.max(i, 1);
            this.#Distances = this.#Resize(this.#Distances);
            this.#Centers = this.#Resize(this.#Centers);
            this.#Indices = this.#Resize(this.#Indices);
            this.#IDs = this.#Resize(this.#IDs);
    ...
    GetImmediates(index)
    {
        return this.#Immediates[this.#Indices[index]];
    }
