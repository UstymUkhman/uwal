 ...
 */
const TEXTURE = /** @type {Usage} */ /*@__PURE__*/ CreateConstantObject(
{
...
export class Texture
{
     ...
     * @property {GPUTexture} [MultisampleTexture]
     * @property {boolean} UseDepthStencilAttachment
     * @property {GPURenderPassDescriptor} RenderPassDescriptor
     *
    ...
    /** @type {number} */ #MipmapsTextures = 0;
    /** @type {Renderer | undefined} */ #Renderer;

    /** @type {GPUTextureFormat} */ #PreferredCanvasFormat;
    /** @type {boolean[]} */ #PipelinesState = [];
    /** @type {RendererState} */ #RendererState =
    {
    ...
    async #CreateMipmaps(texture, descriptor, loop, bindingViewDimension)
    {
        ...
        if (!this.#MipmapsTextures++)
        {
            const { UseDepthStencilAttachment, MultisampleTexture, RenderPassDescriptor } = Renderer;
            this.#RendererState = { UseDepthStencilAttachment, MultisampleTexture, RenderPassDescriptor };

            this.#PipelinesState = Renderer.Pipelines.map(Pipeline =>
            {
        ...
        MipmapsPipeline.SetDrawParams(3, 1);
        MipmapsPipeline.Active = false;
        ...
        if (!--this.#MipmapsTextures)
        {
            const { UseDepthStencilAttachment, MultisampleTexture, RenderPassDescriptor } = this.#RendererState;
            this.#PipelinesState.forEach((state, s) => Renderer.Pipelines[s].Active = state);
     ...
     * [Device.PreferredCanvasFormat](./Device#preferredcanvasformat) and [Usage.RENDER](#usage), respectively.
     */
     ...
     * to [Device.PreferredCanvasFormat](./Device#preferredcanvasformat), [Usage.STORAGE](#usage), and
     * [Renderer.CanvasSize](./Renderer#canvassize) when [Renderer](#renderer) is provided.
     */
