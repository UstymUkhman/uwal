 ...
 * for its usefulness and common pitfalls, and this [example](https://ustymukhman.github.io/uwal-webgpu-fundamentals/dist/#timing-performance)
 * for the basic usage.
 */
export class GPUTiming
{
    ...
    async ResolveAndSubmit()
    {
        ...
        const resultBuffer = this.#ResultBuffers.pop() ?? /*@__INLINE__*/ CreateBuffer(device,
        {
    ...
    /**
     * @throws `ERROR.TIMESTAMP_QUERY_NOT_FOUND` when `"timestamp-query"` feature is not supported.
     * @returns {Promise<GPUQuerySet | undefined>} A query set to use in `Computation|Renderer.CreateTimestampWrites` method.
     */
    get QuerySet()
    {
    ...
    /** @returns {boolean} `true` if `"timestamp-query"` feature is available after initialization, `false` otherwise. */
    get Enabled()
    {
