 ...
 * @property {ShaderModuleDescriptor | ShaderCode} [shader]
 *
 * @typedef {Readonly<{ NONE: 0, COLOR: 1, MAP: 2 }>} Emission
 * @typedef {MaterialOptions | ShaderCode} ShaderOptions
 *
 * @exports MaterialOptions, ShaderOptions
 */
...
export class Material
{
     ...
     * @param {ShaderOptions} options
     * @param {string} [label = "Material"]
     */
    constructor(Renderer, options, label = "Material")
    {
        this.Options = typeof options === "object" && !Array.isArray(options) && options || { shader: options };

        this.Pipeline = new Renderer.Pipeline(
            /** @type {ShaderModuleDescriptor} */ (this.Options.shader || {}).pipelineLabel || label
        );
    ...
    /**
     * @protected
     * @param {MaterialOptions | ShaderCode} [_options]
     * @param {(string | undefined)[]} [defaultShaders]
     */
    GetShaderCode(_options, defaultShaders = [])
    {
        const { shader, emission = 0 } = this.Options;

        const Shader = (Array.isArray(shader) || typeof shader === "string") && shader
