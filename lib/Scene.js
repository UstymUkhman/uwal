...
/**
 * @classdesc Content manager for the `canvas` element. Used to add, search, update and remove {@link Mesh}es,
 * {@link Shape}s, {@link Node}s, lights and {@link Camera}s within the rendering pipeline.
 */
export default class Scene
{
    ...
    /**
     * @description Search for an element in the scene graph where the `Label` member is equal to the argument.
     * @returns {SceneNode | null} Matching element or `null` if not found.
     * @param {string} label
     */
    Find(label)
    {
    ...
    /**
     * @description Add a {@link Camera} to the list. The first added is assigned to the `MainCamera` and all elements
     * are rendererd from its perspective. To switch to a different camera, simply assign it to the `MainCamera` member.
     * @param {Camera} camera
     */
    AddCamera(camera)
    {
    ...
    /**
     * @description Remove a {@link Camera} from the list. If it's assigned to the `MainCamera`, its value is reset.
     * @param {Camera} camera
     */
    RemoveCamera(camera)
    {
     ...
     * Remove all cameras and scene graph elements and reset the `MainCamera` member.
     */
    Destroy()
    {
        this.Traverse(node =>
