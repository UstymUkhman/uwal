 ...
 * @typedef {BaseStage & import("./stages/ComputeStage").default} ComputeStage
 * @typedef {BaseStage & import("./stages/RenderStage").default} RenderStage
 *
 ...
 * @typedef {ComputeStage & { new(): ComputeStage }} Computation
 * @typedef {RenderStage & { new(): RenderStage }} Renderer
 * @typedef {
...
export default class Device
{
     ...
     */
    static Renderer(canvas, programName = "", options = {})
    {
        options.format ??= this.PreferredCanvasFormat;
        this.#SetDescriptorLabel(programName);
        ...
        return (async () =>
        {
            ...
            return new Proxy(Stages.Render,
            {
                construct(Stage)
                {
                    return new Stage(device, programName, canvas, options);
                }
            });
        })();
    }
     ...
     */
    static Computation(programName = "")
    {
        this.#SetDescriptorLabel(programName);
        ...
        return (async () =>
        {
            ...
            return new Proxy(Stages.Compute,
            {
                construct(Stage)
                {
                    return new Stage(device, programName);
                }
            });
        })();
    }
