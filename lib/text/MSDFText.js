...
export default class MSDFText
{
     ...
     * }} [descriptor]
     */
    SetRenderPipeline(Renderer, descriptor)
    {
        let {
            layout,
            module,
        ...
        this.#BindGroupLayout = this.#Pipeline.CreateBindGroupLayout(
            MSDFFont.GetBindGroupLayoutEntries(Renderer).concat(
                this.#Renderer.CreateBufferBindingLayout(
    ...
    {
        !this.#Pipeline && ThrowError(ERROR.PIPELINE_NOT_FOUND, `RenderPipeline.
            Call \`MSDFText.SetRenderPipeline\` method before writing a string.`
        );
    ...
    {
        !this.#Pipeline && ThrowError(ERROR.PIPELINE_NOT_FOUND, `RenderPipeline.
            Call \`MSDFText.SetRenderPipeline\` method before setting the translation matrix.`
        );
    ...
    {
        !this.#Pipeline && ThrowError(ERROR.PIPELINE_NOT_FOUND, `RenderPipeline.
            Call \`MSDFText.SetRenderPipeline\` method before setting text color.`
        );
    ...
    {
        !this.#Pipeline && ThrowError(ERROR.PIPELINE_NOT_FOUND, `RenderPipeline.
            Call \`MSDFText.SetRenderPipeline\` method before setting text scale.`
        );
