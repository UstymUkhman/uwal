...
export class MSDFText
{
    ...
    CreateRenderPipeline(Renderer, descriptor)
    {
        ...
        this.#BindGroupLayout = this.#Pipeline.CreateBindGroupLayout(
            MSDFFont.GetBindGroupLayoutEntries(Renderer).toSpliced(
                2, 0, this.#Renderer.CreateBufferBindingLayout(
     ...
     * @throws `ERROR.PIPELINE_NOT_FOUND` if called before [CreateRenderPipeline](#createrenderpipeline).
     *
    ...
    {
        !this.#Pipeline && ThrowError(ERROR.PIPELINE_NOT_FOUND, `RenderPipeline.
            Call \`MSDFText.CreateRenderPipeline\` method before writing the text.`
        );
        ...
        Pipeline.SetBindGroupFromResources(
            [textBuffer, ...Object.values(Font.BindGroupResources), /** @type {GPUBuffer} */ (this.#CameraBuffer)],
            [2, 0, 1, 3, 10],
