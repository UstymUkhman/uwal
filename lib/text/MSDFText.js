...
export class MSDFText
{
    ...
    /** @param {string} [label = "MSDFText"] - Name of the text */
     ...
     * } [descriptor] - Additional pipeline settings
     */
     ...
     * @param {boolean} [centered = false] - Set to `true` to center the text
     */
    Write(string, color = 0x000000, scale = 0.01, centered = false)
    {
        ...
        !this.#Font && ThrowError(ERROR.FONT_NOT_FOUND, `\`MSDFText.Write\` method.
            Call \`MSDFText.LoadFont\` method or \`MSDFText.Font\` setter before writing a string.`
        );

        !this.#CameraBuffer && ThrowError(ERROR.CAMERA_BUFFER_NOT_FOUND, `\`MSDFText.Write\` method.
            Call \`MSDFText.CameraMatrixBuffer\` setter before writing a string.`
        );
        ...
        if (centered)
        {
            ...
            measurements = this.#Measure(string, Font, ([x, y], _line, c) =>
            {
                text[offset + 0] = x;
                text[offset + 1] = y;
                text[offset + 2] = c;
        ...
        Pipeline.SetBindGroupFromResources([
            textBuffer,
            ...Object.values(Font.BindGroupResources),
            /** @type {GPUBuffer} */ (this.#CameraBuffer)
        ], [0, 1, 2, 3, 10], this.#BindGroupLayout);
     ...
     * @param {GPUBuffer} textBuffer - Text buffer to update
     */
    SetTranslation(translation, textBuffer)
    {
        !this.#Pipeline && ThrowError(ERROR.PIPELINE_NOT_FOUND, `RenderPipeline.
            Call \`MSDFText.CreateRenderPipeline\` method before setting the translation matrix.`
        );
        ...
        /** @type {RenderPipeline} */ (this.#Pipeline).WriteBuffer(
            textBuffer, /** @type {GPUAllowSharedBufferSource} */ (translation), 32, 0, 16
        );
    }
     ...
     * @param {GPUBuffer} [textBuffer] - Text buffer to remove
     */
    Clear(textBuffer)
    {
        textBuffer?.destroy();
    ...
    /** @returns Pipeline used for text rendering. */
    get Pipeline()
    {
     ...
     * @param {MSDFFont} font - [MSDFFont](./MSDFFont) to use
     */
    set Font(font)
    {
        this.#Font = font;
    ...
    /** @returns {MSDFFont | undefined} [MSDFFont](./MSDFFont) instance. */
    get Font()
    {
        return this.#Font;
    }
    ...
    Destroy()
    {
        this.#Pipeline && this.#Renderer?.RemovePipeline(this.#Pipeline);
        this.#Font = this.#Font?.Destroy();
        this.#CameraBuffer = void 0;
