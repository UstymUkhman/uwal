...
export default class SDFText
{
     ...
     * @property {import("../utils/Color").ColorParam} [background = [1, 1, 1, 1]]
     * @property {import("../utils/Color").ColorParam} [color = [0, 0, 0, 1]]
    ...
    #CreateUniformBuffer()
    {
        const { buffer: textBuffer, Text: { matrix: textMatrix, textureSize } } = this.#Renderer.CreateUniformBuffer(
    ...
    #CreateVertexBuffer()
    {
        ...
        this.#VertexBuffers.push(this.#Renderer.CreateVertexBuffer(this.#Vertices));
    }
    ...
    #SetTextRectangle(vertices, origin)
    {
        ...
        this.#Renderer.WriteBuffer(this.#VertexBuffers[0], vertices);

        this.#UpdateMatrix();
    }

    /**
     * @deprecated Use `SDFText.GetShaderStateParams` instead.
    ...
    {
        return SDFText.GetShaderStateParams(Renderer, shader, operation, srcFactor, dstFactor);
    }
     ...
     * @param {GPUBlendFactor} [dstFactor = "one-minus-src1"]
     */
    static async GetShaderStateParams(Renderer, shader = "", operation, srcFactor, dstFactor)
    {
        ...
        const enabled = (await Device.GPUDevice).features.has("dual-source-blending");

        code = `${Shaders.SDFText}\n\n${code}`;
        enabled && (code = `${Shaders.SDFTextDSB}\n\n${code}`);

        const module = Renderer.CreateShaderModule(code, `${SDFText.#Label} Shader Module`);
        const layout = Renderer.CreateVertexBufferLayout(["position", "texture", "size"], void 0, "textVertex");

        return {
            target: Renderer.CreateTargetState(undefined, enabled && { color } || undefined),
            fragmentEntry: (enabled && "dsbTextFragment") || "textFragment",
            constants: { TRIPLET_FACTOR: 0.6 },
            vertexEntry: "textVertex",
            module, layout,
            shader: code
        };
    }
    ...
    async SetFontTexture(source, format = "r8unorm")
    {
        ...
        this.#TextureSize.set([texture.width, texture.height]);
        this.#Renderer.WriteBuffer(this.#TextBuffer, this.#TextureSize);
    ...
    /** @param {import("../utils/Color").ColorParam} color */
    ...
    set Size(size)
    {
        ...
        if (!this.#VertexBuffers.length) return;
        this.#LineGap = this.#Size * lineGap;
